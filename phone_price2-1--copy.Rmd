...
# train data
x_train <- model.matrix(price_range ~ ., df)[train_index, -21]
y_train <- df$price_range[train_index]
data_train <- subset(df[train_index,])

# test data
x_test <- model.matrix(price_range ~ ., df)[-train_index,-21]
y_test <- df$price_range[-train_index]
...
```{r}
set.seed(1)
glm.fit = glm(price_range ~.,data = df, subset = train_index, family = binomial(link = 'logit'))
summary(glm.fit)

#glm by caret
glm.fit = train(x = df[train_index,-c(2,4,6,18:20)],
                y = df$price_range[train_index],
                method = "glm",
                metric = "ROC",
                trControl = ctrl)
summary(glm.fit)
...
## ROC camparison
```{r}
test.pred.prob = predict(glm.fit, newdata = df[-train_index,], type = "response")
roc.glm = roc(df$price_range[-train_index,], test.pred.prob)
```
